  ...
  const cosmosMultiBuy = useCallback(
    async (wallet: Wallet) => {
      ...
      selectedNFTData.map((nft) => {
        ...
        if (nft.networkId !== "furya" || !network) {
      ...
      if (msgs.length > 0) {
        ...
        const cosmwasmClient = await getKeplrSigningCosmWasmClient("furya");
        try {
          ...
          selectedNFTData.map((nft) => {
            setToastSuccess({
              ...
              duration: 10000,
              onPress: () => {
                Linking.openURL(txExplorerLink("furya", tx.transactionHash)); // test it further
              },
            });
