...
import { BuyFuryapunksButton } from "../components/buttonLabel/BuyFuryapunksButton";
...
export const Russian = () => {
  ...
  >("");
  const [totalFuryaUser, setTotalFuryaUser] = useState<number>(0);
  ...
  const {
    data: userFuryaPunksList,
    refetch: handleGetFuryaList,
    error,
  } = useList({
    selectedWallet,
  });
  ...
  const { data: buyTSC, mutate: handleBuyTicket } = useBuyTicket({
    userTokens: [...(userFuryaPunksList as number[])],
    buyCount: bet,
    selectedWallet,
  });
  ...
  useEffect(() => {
    handleGetFuryaList && handleGetFuryaList();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedWallet]);

  const getUserFuryapunks = () => {
    if (Array.isArray(userFuryaPunksList))
      setTotalFuryaUser(userFuryaPunksList.length);
    return 0;
  };
  ...
  const userFuryapunks = totalFuryaUser;
  const maxTicket = "10";
  const priceTicket = "1";
  const monthPriceTicket = lastRewards;
  const remainingUserTicket = remaningTicket;
  const remainingUserCurrency = totalFuryaUser;
  ...
  const userInteractionInfo = result
    ...
    : `${userFuryapunks} Furyapunks in your wallet`;

  const reArrangeTicketList = (tickets: { ticket_id: string }[]) => {
    return tickets.map((ticket: { ticket_id: string }) => ticket.ticket_id);
  };

  const updateFuryaList = () => {
    if (totalFuryaUser - bet <= 0) {
      setTotalFuryaUser(0);
    } else handleGetFuryaList && handleGetFuryaList();
    // getUserFuryapunks();
  };
  ...
  const addBet = () => {
    ...
    if (totalFuryaUser === 0) return setErroType("TICKET");
    if (validateBet <= totalFuryaUser) setBet(validateBet);
  };
  const reduceBet = () => {
    ...
    switch (errorType) {
      ...
      case "NFT":
        return (
          ...
          >
            <BuyFuryapunksButton size={buttonSize} />
