      ...
      try {
        if (addr) {
          const response = await fetch(
            `https://api.roulette.aaa-metahuahua.com/furyapunks?addr=${addr}`
          );
...
export const useBuyTicket = ({
  ...
  const dataBody = { addr, furyapunk: buyArray };
  const URL =
    buyCount === 1
  ...
  const { data, mutate, isError, isLoading } = useMutation(
    ["furyapunk", userTokens, "buyCount", buyCount, "wallet", selectedWallet],
    async () => {
      if (addr) {
