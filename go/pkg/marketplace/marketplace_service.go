...
import (
	...
	"github.com/furysport/furya-dapp/go/internal/airtable_fetcher"
	"github.com/furysport/furya-dapp/go/internal/ethereum"
	...
	case *networks.CosmosNetwork:
		...
		err := s.conf.IndexerDB.Raw(fmt.Sprintf(`
			...
			nft_by_collection AS (
				SELECT  tc.id,n.id  nft_id  FROM furya_collections AS tc
     ...
     trades_by_collection_historic AS (
         ...
         FROM trades t join activities a on a.id = t.activity_id
                       join furya_collections tc on split_part(a.nft_id,'-',2)=tc.mint_contract_address
...
func (s *MarkteplaceService) SearchNames(ctx context.Context, req *marketplacepb.SearchNamesRequest) (*marketplacepb.SearchNamesResponse, error) {
	...
	if err == nil {
		...
		if err := s.conf.IndexerDB.
			Preload("FuryaNFT").
			Joins("JOIN furya_nfts ON furya_nfts.nft_id = nfts.id").
			Where("furya_nfts.token_id ~* ? AND nfts.collection_id = ? AND nfts.burnt = false", req.Input, collectionID).
			Limit(int(limit)).
		...
		for _, nft := range nfts {
			...
			names = append(names, nft.FuryaNFT.TokenID)
		}
	}
...
func (s *MarkteplaceService) SearchCollections(ctx context.Context, req *marketplacepb.SearchCollectionsRequest) (*marketplacepb.SearchCollectionsResponse, error) {
	...
	if err := s.conf.IndexerDB.
		...
		Where("furya_collections.mint_contract_address IN ?", s.conf.Whitelist).
		Limit(int(limit)).
	...
	for _, c := range collections {
		...
		if c.FuryaCollection != nil {
			nc.CreatorId = string(network.GetBase().UserID(c.FuryaCollection.CreatorAddress))
			nc.MintAddress = c.FuryaCollection.MintContractAddress
		}
