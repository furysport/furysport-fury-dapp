...
import (
	...
	"github.com/furysport/furysport-fury-dapp/go/pkg/pricespb"
	"github.com/furysport/furysport-fury-dapp/go/pkg/tmws"
...
func (h *Handler) handleExecuteMint(e *Message, execMsg *wasmtypes.MsgExecuteContract) error {
	...
	if err := h.db.Preload("FuryaCollection").Limit(1).Find(&collections, &indexerdb.Collection{ID: h.config.Network.CollectionID(contractAddress)}).Error; err != nil {
	...
	if collection.FuryaCollection != nil && collection.FuryaCollection.MintContractAddress == h.config.Network.NameServiceContractAddress {
